...
sav.co.ts <- xts(df_sav$CO, dates)
sav.no.ts <- xts(df_sav$NO2, dates)
sav.so.ts <- xts(df_sav$SO2, dates)
sav.pm.ts <- xts(df_sav$PM2_5, dates)
sav.unemp.ts <- xts(sav_unemp$Unemployment_Rate, dates)
...
#Test a Restricted VAR Model
```{r}
mon.var.restrict <- restrict(mon.var)
summary(mon.var.restrict)
```
